...
func shield(delta):
	...
	if degree_state == 0:
		deg = fmod(deg + shielding_speed * delta, 200.0)
	elif degree_state == 1:
		deg = fmod(deg + (-1*shielding_speed) * delta, 200.0)
	
	### shield 1 control
	new_pos_shield1.x = cos(deg2rad(deg) - (PI/1)) # direction to clockwise
	new_pos_shield1.y = sin(deg2rad(deg) - (PI/1)) # direction to clockwise
	new_pos_shield1 = new_pos_shield1 * 100        # start at 0 degree 
	
	shield1.set_rot(deg2rad(-deg) - (3*PI/2))
