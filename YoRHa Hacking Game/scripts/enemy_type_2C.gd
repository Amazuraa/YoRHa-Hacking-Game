...
func rotating_muzzle(delta):
	...
	new_pos_rotation   = new_pos_rotation * 80     # start at 0 degree 
	muzzle1.set_rot(deg2rad(-deg))
	muzzle1.set_pos(new_pos_rotation)
	
	### rotation2 control
	new_pos_rotation2.x = cos(deg2rad(deg) - (PI/2)) # direction to clockwise
	new_pos_rotation2.y = sin(deg2rad(deg) - (PI/2)) # direction to clockwise
	
	new_pos_rotation2  = new_pos_rotation2 * -80
